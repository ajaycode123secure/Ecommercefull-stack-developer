...
export default function Addresses() {
    ...
    const handleSaveAddress = async () => {
        ...
        if (isDefault) {
            ...
            dummyAddress.forEach((a) => {
                a.isDefault = false;
            });
        }

        if (isEditing && editingId) {
            const idx = dummyAddress.findIndex((a) => a._id === editingId);
            if (idx !== -1) {
                dummyAddress[idx] = {
                    ...dummyAddress[idx],
                    type,
            ...
            const newAddress = {
                _id: "addr_" + Math.random().toString(36).substring(2, 11),
                user: "69819801a872c913f84d13f4", // dummy user ID
                type,
                street: street.trim(),
                city: city.trim(),
                state: state.trim(),
    ...
    const handleDeleteAddress = async (id: string) => {
        Alert.alert("Delete Address", "Are you sure you want to delete this address?", [
            { text: "Cancel", style: "cancel" },
            {
                ...
                style: "destructive",
                onPress: async () => {
                    const idx = dummyAddress.findIndex((a) => a._id === id);
                    if (idx !== -1) {
                        dummyAddress.splice(idx, 1);
                    }
                    fetchAddresses();
                }
            }
