...
import { dummyAdminStats, dummyUser, dummyProducts } from "../../../../assets/assets";
...
export default function AdminDashboard() {
    ...
    const fetchStats = async () => {
        const statsWithUsers = {
            ...
            recentOrders: dummyAdminStats.recentOrders.map((order: any) => ({
                ...order,
                user: dummyUser,
            })),
        };
        setStats(statsWithUsers as any);
        setLoading(false);
        setRefreshing(false);
    };
    ...
    return (
        ...
        >
            <View className="mb-8">
                <Text className="text-gray-900 font-bold text-xl mb-4 tracking-tight">Overview</Text>
            ...
            <View className="mb-6">
                <Text className="text-gray-900 font-bold text-xl mb-4 tracking-tight">Recent Orders</Text>
                    ...
                    stats.recentOrders.map((order: any) => (
                        <View key={order._id} className="bg-white p-5 rounded-2xl border border-gray-100 mb-3">
                            <View className="flex-row justify-between items-center mb-3">
                                <View>
                                    <Text className="font-bold text-gray-900 text-base">Total Products : {order.items.reduce((acc: number, item: any) => acc + item.quantity, 0)}</Text>
                                    <Text className="text-gray-500 text-xs mt-1">{new Date(order.createdAt).toLocaleDateString()}</Text>
                                </View>
                            ...
                            <View className="pb-2">
                                {order.items.map((item: any) => (
                                    <Text key={item._id} className="text-gray-600 text-xs mt-1">{item.name} x {item.quantity}</Text>
                                ))}
                            </View>
                            ...
                            <View className="flex-row justify-between items-center">
                                <View className="flex-row items-center">
                                    ...
                                    <Text className="text-gray-600 text-sm">{order.user?.name || 'Unknown User'}</Text>
                                </View>
                                <Text className="text-gray-900 font-bold text-lg">${order.totalAmount.toFixed(2)}</Text>
                            </View>
                        </View>
                    ))
                )}
...
const StatCard = ({ label, value }: { label: string, value: string }) => (
    <View className="bg-white p-5 rounded-2xl border border-gray-100 w-[48%] mb-4 justify-center">
        ...
        <Text className="text-gray-500 text-[10px] font-semibold uppercase tracking-wider">{label}</Text>
    </View>
);
